...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2(
		{allSolutions: _List_Nil, bestStreak: 0, cards: _List_Nil, hintText: '', input: '', message: '点击「新游戏」开始24点挑战！', messageType: $author$project$Main$Info, showAllAnswers: false, showHint: false, skipped: 0, solved: 0, streak: 0, timer: 0, totalGames: 0, totalTime: 0},
...
var $author$project$Main$span = F2(
	function (p, list) {
		if (!list.b) {
			...
			var xs = list.b;
			if (p(x)) {
				var _v1 = A2($author$project$Main$span, p, xs);
				var ys = _v1.a;
				var zs = _v1.b;
				return _Utils_Tuple2(
		...
		switch (msg.$) {
				...
				return $elm$core$List$isEmpty(solutions) ? _Utils_Tuple2(model, $author$project$Main$generateCards) : _Utils_Tuple2(
					_Utils_update(
						model,
						{
							...
							hintText: '',
							input: '',
							message: (!model.totalGames) ? '请用下面4张牌算出24点！' : '新的一组牌！',
			...
			case 'SubmitAnswer':
				var cardValues = A2(
					...
					model.cards);
				var _v1 = A2($author$project$Main$parseAndValidate, model.input, cardValues);
				if (_v1.$ === 'Ok') {
					var result = _v1.a;
					if ($elm$core$Basics$abs(result - 24) < 0.0001) {
						var newStreak = model.streak + 1;
						var newBest = A2($elm$core$Basics$max, newStreak, model.bestStreak);
						var newModel = _Utils_update(
							model,
							{bestStreak: newBest, input: '', message: '🎉 正确！「' + (model.input + ' = 24」'), messageType: $author$project$Main$Success, showHint: false, solved: model.solved + 1, streak: newStreak});
						return _Utils_Tuple2(
							newModel,
							$elm$core$Platform$Cmd$batch(
								_List_fromArray(
						...
						return _Utils_Tuple2(errModel, $elm$core$Platform$Cmd$none);
					}
				} else {
					var errMsg = _v1.a;
					var newModel = _Utils_update(
			...
			case 'ShowHint':
				...
				if (!_v2.b) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{hintText: first, message: '💡 提示已显示', messageType: $author$project$Main$Info, showHint: true}),
						$elm$core$Platform$Cmd$none);
				}
			case 'ShowAllAnswers':
				return _Utils_Tuple2(
			...
			case 'NewGame':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{message: '新游戏开始！', messageType: $author$project$Main$Info, showAllAnswers: false, streak: 0, timer: 0}),
					$author$project$Main$generateCards);
			case 'Skip':
				var newModel = _Utils_update(
					model,
...
var $author$project$Main$css = A3(
	$elm$html$Html$node,
	'style',
	_List_Nil,
	_List_fromArray(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A2($elm$html$Html$Attributes$style, 'border', '2px solid ' + card.color)
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('card-display'),
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('card-suit'),
						A2($elm$html$Html$Attributes$style, 'color', card.color)
					]),
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('card-value-bottom'),
						A2($elm$html$Html$Attributes$style, 'color', card.color)
					]),
				_List_fromArray(
					[
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('用加减乘除和括号，让四张牌算出24')
							]))
					])),
				A2(
				$elm$html$Html$div,
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text(
										$elm$core$String$fromInt(model.skipped))
									]))
							])),
						A2(
						$elm$html$Html$div,
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text(
										$author$project$Main$formatTime(model.totalTime))
									]))
							])),
						A2(
						$elm$html$Html$div,
								...
								_List_fromArray(
									[
										...
										function () {
											...
											return (!total) ? '0%' : ($elm$core$String$fromInt(
				...
				A2($elm$core$List$map, $author$project$Main$viewCard, model.cards)),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('💡 参考解法：' + model.hintText)
					])) : $elm$html$Html$text(''),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					...
					[
						...
						_List_fromArray(
							[
								...
								$elm$html$Html$Attributes$placeholder('输入算式，如 (3+3)*8/2'),
								...
								A2(
									$elm$json$Json$Decode$map,
									function (key) {
										return (key === 13) ? $author$project$Main$SubmitAnswer : $author$project$Main$NoOp;
									},
					...
					[
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text('支持分数运算，如 (8/(3-8/3))*3 = 24')
									]))
							])),
						A2(
						$elm$html$Html$p,
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text('牌面 4, 4, 10, 10：'),
										A2(
										$elm$html$Html$code,
										_List_Nil,
										_List_fromArray(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('用 Elm 语言构建 · 纯函数式编程 · 无运行时错误')
					]))
			]));
};
